...
while [[ $# -gt 0 ]]; do
    case $1 in
        -v|--verb)
            FILTER_CMD="$FILTER_CMD | grep '\"verb\": \"$2\"'"
            shift 2
            ;;
        -s|--status)
            FILTER_CMD="$FILTER_CMD | grep '\"code\": $2'"
            shift 2
            ;;
        -u|--user)
            FILTER_CMD="$FILTER_CMD | grep '\"username\": \"$2\"'"
            shift 2
            ;;
        -n|--namespace)
            FILTER_CMD="$FILTER_CMD | grep '\"namespace\": \"$2\"'"
            shift 2
            ;;
        -r|--resource)
            FILTER_CMD="$FILTER_CMD | grep '\"resource\": \"$2\"'"
            shift 2
            ;;
        -a|--allowed)
            FILTER_CMD="$FILTER_CMD | grep '\"decision\": \"allow\"'"
            shift
            ;;
        -d|--denied)
            FILTER_CMD="$FILTER_CMD | grep '\"decision\": \"forbid\"'"
            shift
            ;;
        -g|--group)
            FILTER_CMD="$FILTER_CMD | grep '\"groups\":.*$2'"
            shift 2
            ;;
        -i|--ip)
            FILTER_CMD="$FILTER_CMD | grep '\"sourceIPs\":.*$2'"
            shift 2
            ;;
        -h|--help)
            usage
